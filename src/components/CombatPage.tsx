...
export function CombatPage({ combat, bestiary, readOnly, currentUser, onCombatChange, onSaveCreature, focusRoute = "" }: CombatPageProps) {
  ...
  const [enemyAttackMediaTarget, setEnemyAttackMediaTarget] = useState<{ enemyId: string; attackId: string } | null>(null);
  ...
  return (
    <div className="combat-page">
      ...
      {enemyAttackMediaTarget && <EnemyAttackMediaManager enemy={normalized.enemies.find((item) => item.id === enemyAttackMediaTarget.enemyId) || null} attackId={enemyAttackMediaTarget.attackId} onClose={() => setEnemyAttackMediaTarget(null)} onSave={(enemy) => { updateEnemy(enemy); setEnemyAttackMediaTarget(null); }} />}
    </div>
  );
}
...
function CombatHome({ bosses, enemies, bestiary, onOpen, onOpenBoss, onOpenEnemy }: { bosses: CombatBoss[]; enemies: CombatEnemy[]; bestiary: BestiaryCreature[]; onOpen: (section: CombatSection) => void; onOpenBoss: (id: string) => void; onOpenEnemy: (id: string) => void }) {
  ...
  const aggregate = bossAttacks.length
    ...
    : 0;
  const categories = [
    { title: "Bosses", icon: "Crown", count: bossCount, description: "Encounter chapters, phases, movesets, production status, and implementation handoff.", action: () => onOpen("bosses"), image: bosses.find((boss) => boss.artwork?.imageUrl || boss.artwork?.spriteAnimation)?.artwork, meta: `${aggregate}% production health` },
    { title: "Enemies", icon: "Skull", count: enemies.length, description: "Enemy families, elites, coordinated behaviors, attack kits, and production requirements.", action: () => onOpen("enemies"), image: enemies.map((enemy) => enemyCoverAsset(enemy, bestiary)).find((asset) => asset.imageUrl || asset.spriteAnimation), meta: `${enemies.filter((enemy) => enemy.tier === "Elite").length} elite` },
  ...
  return <>
    ...
    <section className="combat-category-grid">
      {categories.map((category) => <button key={category.title} className="combat-category-card dashboard-category-box-frame" onClick={category.action} disabled={!category.action}>
        <span className="combat-category-visual">{category.image?.imageUrl || category.image?.spriteAnimation ? <SpriteAwareImage src={category.image.imageUrl} spriteAnimation={category.image.spriteAnimation} imageFit={category.image.imageFit} alt="" /> : <Icon name={category.icon} className="h-9 w-9" />}</span>
    ...
    <section className="combat-recent-section">
      ...
      <div className="combat-recent-grid">{recent.map((item) => <button key={`${item.kind}-${item.id}`} onClick={item.onOpen}><span>{item.image?.imageUrl || item.image?.spriteAnimation ? <SpriteAwareImage src={item.image.imageUrl} spriteAnimation={item.image.spriteAnimation} imageFit={item.image.imageFit} alt="" /> : <Icon name={item.icon} className="h-5 w-5" />}</span><div><b>{item.name}</b><small>{item.kind} · {formatUpdatedAt(item.updatedAt)}</small></div><Icon name="ChevronRight" className="h-4 w-4" /></button>)}{!recent.length && <p className="combat-muted">Combat activity will appear here as the library grows.</p>}</div>
    </section>
  </>;
}
...
function EnemyCard({ enemy, bestiary, readOnly, onOpen, onEdit, onDelete, onSaveImage }: { enemy: CombatEnemy; bestiary: BestiaryCreature[]; readOnly: boolean; onOpen: () => void; onEdit: () => void; onDelete: () => void; onSaveImage: (asset: EnemyCoverAsset, next: { imageUrl: string; imageFit: ImageFitSettings }) => void }) {
  ...
  return <AssignableModule as="article" className={`combat-enemy-card lore-card-frame ${enemy.tier === "Elite" ? "elite" : ""}`} module={assignmentModule}>
    <button onClick={onOpen}>
      <span className="combat-enemy-art">{asset.spriteAnimation ? <SpriteAwareImage src={asset.imageUrl} spriteAnimation={asset.spriteAnimation} imageFit={asset.imageFit} alt={enemy.name} /> : asset.imageUrl ? <RepositionableImage src={asset.imageUrl} alt={enemy.name} label={`${enemy.name} card image`} imageFit={asset.imageFit} aspectRatio="16 / 9" canReposition={!readOnly} assignmentModule={assignmentModule} onSave={(next) => onSaveImage(asset, next)} /> : <Icon name={enemy.tier === "Elite" ? "ShieldAlert" : "Bug"} className="h-11 w-11" />}</span>
...
function EnemyWorkspace({ enemy, bestiary, readOnly, onSaveImage, onBack, onEdit, onManageMedia, onAddAttack, onEditAttack, onManageAttackMedia, onDeleteAttack, onMoveAttack }: { enemy: CombatEnemy; bestiary: BestiaryCreature[]; readOnly: boolean; onSaveImage: (asset: EnemyCoverAsset, next: { imageUrl: string; imageFit: ImageFitSettings }) => void; onBack: () => void; onEdit: () => void; onManageMedia: () => void; onAddAttack: () => void; onEditAttack: (attack: CombatAttack) => void; onManageAttackMedia: (attackId: string) => void; onDeleteAttack: (attackId: string) => void; onMoveAttack: (attackId: string, direction: -1 | 1) => void }) {
  const creature = linkedBestiaryCreature(enemy, bestiary);
  ...
  return <main className="combat-enemy-workspace">
    ...
    <header className={`combat-enemy-hero ${enemy.tier === "Elite" ? "elite" : ""}`}>
      <div className="combat-enemy-hero-art">{cover.spriteAnimation ? <SpriteAwareImage src={cover.imageUrl} spriteAnimation={cover.spriteAnimation} imageFit={cover.imageFit} alt={enemy.name} /> : cover.imageUrl ? <RepositionableImage src={cover.imageUrl} alt={enemy.name} label={`${enemy.name} hero image`} imageFit={cover.imageFit} aspectRatio="1 / 1" canReposition={!readOnly} onSave={(next) => onSaveImage(cover, next)} /> : <Icon name={enemy.tier === "Elite" ? "ShieldAlert" : "Bug"} className="h-16 w-16" />}</div>
    ...
    <section className="combat-workspace-section combat-enemy-moves"><div className="combat-enemy-moves-heading"><SectionHeading eyebrow={`${enemy.tier} Kit`} title="Moveset" />{!readOnly && <button className="button-frame combat-action" onClick={onAddAttack}><Icon name="Plus" className="h-4 w-4" /> Add Move</button>}</div><div className="combat-enemy-move-grid">{enemy.attacks.map((attack, index) => <EnemyMoveCard key={attack.id} attack={attack} visual={attackVisual(attack, assets)} index={index} readOnly={readOnly} canMoveUp={index > 0} canMoveDown={index < enemy.attacks.length - 1} onEdit={() => onEditAttack(attack)} onManageMedia={() => onManageAttackMedia(attack.id)} onDelete={() => onDeleteAttack(attack.id)} onMove={(direction) => onMoveAttack(attack.id, direction)} />)}</div>{!enemy.attacks.length && <div className="combat-empty-inline">No moves documented yet. Add the enemy's first move to begin its production kit.</div>}</section>
    <section className="combat-enemy-overview-grid">
      <article className="combat-reading-panel"><SectionHeading eyebrow="Animation Handoff" title="Required Motion" /><p>{enemy.animationNotes}</p><div className="combat-clip-list">{Array.from(new Set(enemy.attacks.flatMap((attack) => attack.animation.requiredClips))).map((clip) => <code key={clip}>{clip}</code>)}</div></article>
    ...
    <section className="combat-workspace-section"><SectionHeading eyebrow="Bestiary & Art Binder" title="Linked Visuals" /><div className="combat-enemy-media-grid">{assets.map((asset) => <article key={asset.id}><span><SpriteAwareImage src={asset.imageUrl} spriteAnimation={asset.spriteAnimation} imageFit={asset.imageFit} alt={asset.label} /></span><small>{asset.section}</small><b>{asset.label}</b>{asset.spriteAnimation && <em>Live animation</em>}</article>)}{!assets.length && <div className="combat-empty-inline">Images and sprite animations uploaded to {enemy.name}'s Bestiary or Art Binder will appear here automatically.</div>}</div></section>
  </main>;
}

function EnemyMoveCard({ attack, visual, index, readOnly, canMoveUp, canMoveDown, onEdit, onManageMedia, onDelete, onMove }: { attack: CombatAttack; visual: CreatureArtAsset | null; index: number; readOnly: boolean; canMoveUp: boolean; canMoveDown: boolean; onEdit: () => void; onManageMedia: () => void; onDelete: () => void; onMove: (direction: -1 | 1) => void }) {
  const media = attack.media.find((item) => item.spriteAnimation) || attack.media.find((item) => item.imageUrl);
  return <article className={`combat-enemy-move-card ${attack.specialAttack ? "is-special-attack" : ""}`}>
    <div className="combat-enemy-move-preview"><SpriteAwareImage src={media?.imageUrl || visual?.imageUrl} spriteAnimation={media?.spriteAnimation || visual?.spriteAnimation} imageFit={media?.imageFit || visual?.imageFit} alt={`${attack.name} animation`} fallback={<div><Icon name="Film" className="h-8 w-8" /><span>Add animation</span></div>} /></div>
    {attack.specialAttack && <SpecialAttackBadge />}
    <header><span>{String(index + 1).padStart(2, "0")}</span><div><small>{attack.damageType || "Combat move"}</small><h3 className="font-display">{attack.name}</h3><code>{attack.internalId}</code></div>{!readOnly && <div className="combat-row-actions combat-enemy-move-actions"><button onClick={() => onMove(-1)} disabled={!canMoveUp} title="Move up"><Icon name="ChevronUp" className="h-4 w-4" /></button><button onClick={() => onMove(1)} disabled={!canMoveDown} title="Move down"><Icon name="ChevronDown" className="h-4 w-4" /></button><button onClick={onManageMedia} title="Manage animation and media"><Icon name="Film" className="h-4 w-4" /></button><button onClick={onEdit} title="Edit move"><Icon name="Edit3" className="h-4 w-4" /></button><button className="danger" onClick={onDelete} title="Delete move"><Icon name="Trash2" className="h-4 w-4" /></button></div>}</header>
    <p>{attack.summary}</p>
    <div className="combat-enemy-move-values"><span><small>Speed</small><b>{formatSpeed(attack.movementSpeed)}</b></span><span><small>Startup</small><b>{formatSeconds(attack.startup)}</b></span><span><small>Cooldown</small><b>{formatSeconds(attack.cooldown)}</b></span><span><small>Range</small><b>{attack.range}</b></span><span><small>Response</small><b>{[...attack.expectedResponses, attack.customResponse].filter(Boolean).join(" · ")}</b></span></div>
...
function linkedBossBestiaryCreature(boss: CombatBoss, bestiary: BestiaryCreature[]) {
  const normalizedName = normalizedCreatureName(boss.name);
  return bestiary.find((creature) => normalizedCreatureName(creature.name) === normalizedName) || null;
}
...
function attackVisual(attack: CombatAttack, assets: CreatureArtAsset[]): CreatureArtAsset | null {
  const attackTerms = combatVisualTerms([
    attack.name,
    attack.internalId,
    attack.animation.assetName,
    ...attack.animation.requiredClips
  ]);
  let best: { asset: CreatureArtAsset; score: number } | null = null;
  for (const asset of assets) {
    const assetTerms = combatVisualTerms([asset.label, asset.section]);
    const termScore = Array.from(assetTerms).reduce((total, term) => total + (attackTerms.has(term) ? (term.length >= 6 ? 3 : 1) : 0), 0);
    const score = termScore > 0 ? termScore + (asset.spriteAnimation ? 2 : 0) : 0;
    if (score > 0 && (!best || score > best.score)) best = { asset, score };
  }
  return best?.asset || null;
}

function combatVisualTerms(values: string[]) {
  const ignored = new Set(["animation", "sprite", "sheet", "sprites", "combat", "enemy", "creature", "pose", "loop", "start", "end", "the", "and", "for"]);
  return new Set(values.flatMap((value) => String(value || "").replace(/([a-z])([A-Z])/g, "$1 $2").toLowerCase().split(/[^a-z0-9]+/)).filter((term) => term.length >= 3 && !ignored.has(term)));
}
...
function enemyCoverAsset(enemy: CombatEnemy, bestiary: BestiaryCreature[]): EnemyCoverAsset {
  if (enemy.artwork?.imageUrl || enemy.artwork?.spriteAnimation) return { imageUrl: enemy.artwork.imageUrl, imageFit: enemy.artwork.imageFit, spriteAnimation: enemy.artwork.spriteAnimation, source: "combat", slot: "artwork" };
  const creature = linkedBestiaryCreature(enemy, bestiary);
  ...
  if (art) return { imageUrl: art.imageUrl, imageFit: art.imageFit, spriteAnimation: art.spriteAnimation, source: "bestiary", slot: "artVault", sectionId: art.sectionId, slotId: art.slotId };
...
function BossCard({ boss, readOnly, onOpen, onDelete, onSaveArtwork }: { boss: CombatBoss; readOnly: boolean; onOpen: () => void; onDelete: () => void; onSaveArtwork: (next: { imageUrl: string; imageFit: ImageFitSettings }) => void }) {
  ...
  return <AssignableModule as="article" className="combat-boss-card lore-card-frame" module={assignmentModule}>
    <button className="combat-boss-open" onClick={onOpen}>
      <span className="combat-boss-art">{boss.artwork?.spriteAnimation ? <CombatMediaVisual media={boss.artwork} alt={boss.name} /> : boss.artwork?.imageUrl ? <RepositionableImage src={boss.artwork.imageUrl} alt={boss.name} label={`${boss.name} boss card image`} imageFit={boss.artwork.imageFit} aspectRatio="16 / 10" canReposition={!readOnly} assignmentModule={assignmentModule} onSave={onSaveArtwork} /> : <Icon name="Crown" className="h-10 w-10" />}</span>
  ...
  return <div className="combat-boss-layout">
    ...
    <main className="combat-boss-main">
      ...
      <header className="combat-boss-hero">
        <div className="combat-boss-hero-art">{boss.artwork?.spriteAnimation ? <CombatMediaVisual media={boss.artwork} alt={boss.name} /> : boss.artwork?.imageUrl ? <RepositionableImage src={boss.artwork.imageUrl} alt={boss.name} label={`${boss.name} hero artwork`} imageFit={boss.artwork.imageFit} aspectRatio="16 / 10" canReposition={!props.readOnly} onSave={props.onSaveArtwork} /> : <Icon name="Crown" className="h-14 w-14" />}</div>
...
function Moveset({ boss, bestiary, totalAttacks, readOnly, onAddPhase, onEditPhase, onManagePhaseMedia, onDeletePhase, onMovePhase, onAddAttack, onEditAttack, onOpenAttack, onDeleteAttack, onMoveAttack, view, onViewChange }: Parameters<typeof BossWorkspace>[0] & { totalAttacks: number; view: "visual" | "production"; onViewChange: (view: "visual" | "production") => void }) {
  ...
  return <>
    ...
    <div className="combat-phase-list">{boss.phases.map((phase, phaseIndex) => <section key={phase.id} id={`combat-phase-${phase.id}`} className="combat-phase-section">
      ...
      {view === "visual" ? <div className="combat-attack-grid">{phase.attacks.map((attack, attackIndex) => <AttackCard key={attack.id} attack={attack} visual={attackVisual(attack, visualAssets)} boss={boss} phase={phase} index={attackIndex} readOnly={readOnly} onOpen={() => onOpenAttack(phase.id, attack.id)} onEdit={() => onEditAttack(phase, attack)} onDelete={() => onDeleteAttack(phase, attack.id)} onMove={(direction) => onMoveAttack(phase, attack.id, direction)} />)}</div> : <CombatProductionTable boss={boss} phase={phase} readOnly={readOnly} onOpenAttack={onOpenAttack} onEditAttack={onEditAttack} onDeleteAttack={onDeleteAttack} />}
...
function AttackCard({ attack, visual, boss, phase, index, readOnly, onOpen, onEdit, onDelete, onMove }: { attack: CombatAttack; visual: CreatureArtAsset | null; boss: CombatBoss; phase: CombatPhase; index: number; readOnly: boolean; onOpen: () => void; onEdit: () => void; onDelete: () => void; onMove: (direction: -1 | 1) => void }) {
  const preview = attack.media.find((item) => item.spriteAnimation) || attack.media.find((item) => item.imageUrl);
...
function AttackWorkspace({ attack, boss, phase, bestiary, readOnly, currentUser, onBack, onOpenBossTab, onOpenAttack, onEdit, onManageMedia, onSave }: { attack: CombatAttack; boss: CombatBoss; phase: CombatPhase; bestiary: BestiaryCreature[]; readOnly: boolean; currentUser: GoogleAccountUser; onBack: () => void; onOpenBossTab: (tab: BossTab) => void; onOpenAttack: (phaseId: string, attackId: string) => void; onEdit: () => void; onManageMedia: () => void; onSave: (attack: CombatAttack) => void }) {
  const [comment, setComment] = useState("");
  const [navOpen, setNavOpen] = useState(false);
  const [collaborationOpen, setCollaborationOpen] = useState(false);
  const [collaborationTab, setCollaborationTab] = useState<"Discussion" | "Activity">("Discussion");
  const linkedVisual = attackVisual(attack, creatureArtAssets(linkedBossBestiaryCreature(boss, bestiary)));
...
function AttackMediaHero({ attack, linkedVisual, readOnly, onManageMedia }: { attack: CombatAttack; linkedVisual: CreatureArtAsset | null; readOnly: boolean; onManageMedia: () => void }) {
  const [selectedId, setSelectedId] = useState(attack.media.find((item) => item.spriteAnimation || item.imageUrl)?.id || attack.media[0]?.id || "");
  useEffect(() => setSelectedId(attack.media.find((item) => item.spriteAnimation || item.imageUrl)?.id || attack.media[0]?.id || ""), [attack.id]);
  const selected = attack.media.find((item) => item.id === selectedId) || attack.media[0];
  return <section className="combat-workspace-section combat-media-dossier"><SectionHeading eyebrow="Visual Handoff" title="Attack Preview & Storyboard" /><div className="combat-media-stage"><div className="combat-media-selected">{selected?.spriteAnimation || selected?.imageUrl ? <CombatMediaVisual media={selected} alt={selected.label} /> : linkedVisual ? <SpriteAwareImage src={linkedVisual.imageUrl} spriteAnimation={linkedVisual.spriteAnimation} imageFit={linkedVisual.imageFit} alt={`${attack.name} animation`} /> : <div><Icon name="Frame" className="h-10 w-10" /><b>No visual selected</b><span>Add a rough sketch, storyboard, keyframe, or live sprite animation.</span></div>}</div><aside>{selected ? <><small>{selected.kind}{selected.timestamp ? ` · ${selected.timestamp}` : ""}{selected.spriteAnimation ? " · Live animation" : ""}</small><h3 className="font-display">{selected.label}</h3><p>{selected.notes || "No notes for this visual."}</p>{selected.webViewLink && <a href={selected.webViewLink} target="_blank" rel="noreferrer"><Icon name="ExternalLink" className="h-4 w-4" /> Open Drive source</a>}</> : linkedVisual ? <><small>Cookbook Art Binder · {linkedVisual.spriteAnimation ? "Live animation" : "Linked visual"}</small><h3 className="font-display">{linkedVisual.label}</h3><p>This visual is linked automatically from the matching Bestiary Art Binder slot.</p></> : <p className="combat-muted">Media metadata will appear here.</p>}{!readOnly && <button className="button-frame combat-action" onClick={onManageMedia}><Icon name="Image" className="h-4 w-4" /> Manage Media</button>}</aside></div><div className="combat-media-thumbnails">{attack.media.map((item, index) => <button key={item.id} className={selected?.id === item.id ? "active" : ""} onClick={() => setSelectedId(item.id)}><span>{item.spriteAnimation || item.imageUrl ? <CombatMediaVisual media={item} alt="" /> : <Icon name="Image" className="h-5 w-5" />}</span><small>{item.timestamp || `Frame ${String(index + 1).padStart(2, "0")}`}</small><b>{item.label}</b></button>)}{!attack.media.length && linkedVisual && <div className="combat-linked-animation-thumb"><span><SpriteAwareImage src={linkedVisual.imageUrl} spriteAnimation={linkedVisual.spriteAnimation} imageFit={linkedVisual.imageFit} alt="" /></span><small>Art Binder</small><b>{linkedVisual.label}</b></div>}</div></section>;
}
...
function GameplayValueGroups({ attack }: { attack: CombatAttack }) {
  const groups: { title: string; icon: string; values: [string, unknown][] }[] = [
    ...
    { title: "Spatial", icon: "Move", values: [["Range", attack.range], ["Movement speed", formatSpeed(attack.movementSpeed)]] }
  ];
...
function CombatMediaManager({ target, bosses, onClose, onSave }: { target: Exclude<MediaTarget, null>; bosses: CombatBoss[]; onClose: () => void; onSave: (boss: CombatBoss) => void }) {
  ...
  const sourceMedia: CombatMediaReference[] = target.type === "boss"
    ...
    : target.type === "phase"
      ...
      : attack!.media.length ? attack!.media : [{ id: `attack-animation-${attack!.id}`, kind: "Animation Reference", label: `${attack!.name} Animation`, imageUrl: "", notes: "", order: 0 }];
  ...
  return <ImageManagerModal title={mediaTitle} subtitle="Uses the Cookbook's shared Google Drive image and sprite animation workflow." slots={sourceMedia.map((media) => ({ id: media.id, label: media.label, description: `${media.kind}${media.timestamp ? ` · ${media.timestamp}` : ""}${media.notes ? ` · ${media.notes}` : ""}`, imageUrl: media.imageUrl, imageFit: media.imageFit, webViewLink: media.webViewLink, spriteAnimation: media.spriteAnimation, aspectRatio: target.type === "boss" && media.id === "boss-artwork" ? "16 / 10" : "16 / 9", uploadNameContext: { subjectName: boss.name, categoryName: target.type === "boss" ? "Combat Boss Art" : target.type === "phase" ? `Combat Phase - ${phase!.name}` : `Combat Attack - ${attack!.name}`, slotName: media.label } }))} onClose={onClose} onSave={(slots) => {
...
function EnemyMediaManager({ enemy, onClose, onSave }: { enemy: CombatEnemy; onClose: () => void; onSave: (enemy: CombatEnemy) => void }) {
  ...
  return <ImageManagerModal title={`${enemy.name} · Artwork & References`} subtitle="Uses the Cookbook's shared Google Drive image and sprite animation workflow." slots={sourceMedia.map((media, index) => ({ id: media.id, label: media.label, description: `${media.kind}${media.notes ? ` · ${media.notes}` : ""}`, imageUrl: media.imageUrl, imageFit: media.imageFit, webViewLink: media.webViewLink, spriteAnimation: media.spriteAnimation, aspectRatio: index === 0 ? "16 / 9" : "16 / 9", uploadNameContext: { subjectName: enemy.name, categoryName: index === 0 ? "Combat Enemy Art" : "Combat Enemy References", slotName: media.label } }))} onClose={onClose} onSave={(slots) => {
    const nextMedia = mergeMedia(sourceMedia, slots);
    onSave({ ...enemy, artwork: nextMedia[0], references: nextMedia.slice(1) });
  }} />;
}

function EnemyAttackMediaManager({ enemy, attackId, onClose, onSave }: { enemy: CombatEnemy | null; attackId: string; onClose: () => void; onSave: (enemy: CombatEnemy) => void }) {
  const attack = enemy?.attacks.find((item) => item.id === attackId) || null;
  if (!enemy || !attack) return null;
  const sourceMedia: CombatMediaReference[] = attack.media.length ? attack.media : [{ id: `enemy-attack-animation-${attack.id}`, kind: "Animation Reference", label: `${attack.name} Animation`, imageUrl: "", notes: "", order: 0 }];
  return <ImageManagerModal title={`${enemy.name} · ${attack.name}`} subtitle="Upload a visual or use Make Sprite Animation to connect the live move animation." slots={sourceMedia.map((media) => ({ id: media.id, label: media.label, description: `${media.kind}${media.notes ? ` · ${media.notes}` : ""}`, imageUrl: media.imageUrl, imageFit: media.imageFit, webViewLink: media.webViewLink, spriteAnimation: media.spriteAnimation, aspectRatio: "16 / 9", uploadNameContext: { subjectName: enemy.name, categoryName: `Combat Move - ${attack.name}`, slotName: media.label } }))} onClose={onClose} onSave={(slots) => {
    const media = mergeMedia(sourceMedia, slots);
    onSave({ ...enemy, attacks: enemy.attacks.map((item) => item.id === attack.id ? { ...item, media, updatedAt: new Date().toISOString() } : item) });
  }} />;
}
...
function SpecialAttackBadge({ compact = false }: { compact?: boolean }) { return <span className={`combat-special-attack-badge ${compact ? "compact" : ""}`} title="Magic used during the wind-up interrupts this attack and stuns the enemy"><Icon name="Sparkles" className="h-4 w-4" /><b>Special Attack</b>{!compact && <small>Magic interrupt stuns</small>}</span>; }
...
function ReferenceGallery({ media, empty, onManage }: { media: CombatMediaReference[]; empty: string; onManage?: () => void }) { return <div className="combat-reference-gallery">{media.map((item) => <figure key={item.id}><span>{item.spriteAnimation || item.imageUrl ? <CombatMediaVisual media={item} alt={item.label} /> : <Icon name="Image" className="h-7 w-7" />}</span><figcaption><small>{item.kind}{item.timestamp ? ` · ${item.timestamp}` : ""}{item.spriteAnimation ? " · Live animation" : ""}</small><b>{item.label}</b>{item.notes && <p>{item.notes}</p>}</figcaption></figure>)}{!media.length && <div className="combat-reference-empty"><Icon name="Image" className="h-7 w-7" /><p>{empty}</p></div>}{onManage && <button className="combat-add-reference" onClick={onManage}><Icon name="Image" className="h-4 w-4" /> Manage Media</button>}</div>; }
function CombatMediaVisual({ media, alt }: { media: CombatMediaReference; alt: string }) { return <SpriteAwareImage src={media.imageUrl} spriteAnimation={media.spriteAnimation} imageFit={media.imageFit} alt={alt} />; }
...
function mergeMedia(source: CombatMediaReference[], slots: ImageManagerSlotDraft[]) { return source.map((item) => { const slot = slots.find((candidate) => candidate.id === item.id); return slot ? { ...item, imageUrl: slot.imageUrl, imageFit: slot.imageFit, webViewLink: slot.webViewLink, spriteAnimation: slot.spriteAnimation } : item; }); }
...
function formatSpeed(value?: number) { return value === undefined ? "—" : `${value} m/s`; }
