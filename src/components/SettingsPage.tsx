...
export function SettingsPage({
  ...
  const exportJson = () => {
    ...
    link.href = href;
    link.download = `world-scribe-codex-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  const exportWebsiteDataJson = () => {
    ...
    link.click();
    URL.revokeObjectURL(href);
    setMessage("Downloaded lore-data.json for a static read-only viewer.");
  };
  ...
  const exportShareableHtml = () => {
    ...
    const link = document.createElement("a");
    link.href = href;
    link.download = `world-scribe-codex-share-${new Date().toISOString().slice(0, 10)}.html`;
    link.click();
    URL.revokeObjectURL(href);
    setMessage("Downloaded a read-only HTML snapshot.");
  };
  ...
  const openLiveViewCopy = () => {
    ...
    setMessage("Opened a read-only local view copy in a new tab.");
  };
  ...
  const uploadLogo = async (file: File | undefined) => {
    ...
    try {
      ...
      setMessage("Logo image saved locally.");
    } catch (error) {
    ...
    <div className="space-y-5 p-4 md:p-6">
      <section className="category-header-frame rounded p-5">
        ...
        <p className="mt-2 max-w-3xl leading-7" style={{ color: "var(--muted-ink)" }}>
          Local data tools, theme, storage, logo, backups, and Scribe AI backend status.
        </p>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="soft-panel rounded p-4">
          ...
          <p className="mt-3 text-sm" style={{ color: "var(--muted-ink)" }}>
            This app saves to this browser. Export JSON when you want a portable backup.
          </p>
        </div>
        ...
        <div className="soft-panel rounded p-4">
          ...
          <p className="mt-3 text-sm" style={{ color: "var(--muted-ink)" }}>
            Large uploaded files may exceed browser storage limits. Image links are best for large art boards.
          </p>
        </div>
      </section>
